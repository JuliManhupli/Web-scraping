...
def spider_quote(url):
    ...
    if response.status_code == 200:
        ...
        for quote in content:
            text = quote.find('span', class_='text').text  # текст цитати
            author = quote.find('small', class_='author').text  # автор
            tags = quote.find('div', class_='tags').meta['content'].split(',')  # теги
...
def main():
    ...
    with open(os.path.abspath('data/quotes.json'), 'w', encoding='utf-8') as fd:
        json.dump(data_quotes, fd, ensure_ascii=False, indent=2)
    with open(os.path.abspath('data/authors.json'), 'w', encoding='utf-8') as fd:
